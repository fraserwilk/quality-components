...
- `src/sass/theme/` — Custom SCSS partials (`_child_theme_variables.scss`, `_buttons.scss`, `_footer.scss`, `_single-product.scss`)
...
- Product specs label mapping via `get_product_specs_labels()` in `functions.php`
- WooCommerce product tab switching JS (inline, in `functions.php`)
